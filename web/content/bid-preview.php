<?php
	...
	if(isset($_GET['id'])) {
		...
		$evaluationID = $aes->decrypt();
	}

	if(isset($_GET['invoice'])) {
		$invoiceArray = $_GET['invoice'];
	}

	if(isset($_GET['bidAcceptance'])) {
		$bidAcceptanceArray = $_GET['bidAcceptance'];
	}

	if(isset($_GET['projectComplete'])) {
		$projectCompleteArray = $_GET['projectComplete'];
	}

	if(isset($_GET['bidTotal'])) {
		$bidTotal = $_GET['bidTotal'];
	}

	if(isset($_GET['bidSubTotal'])) {
		$bidSubTotal = $_GET['bidSubTotal'];	
	}
	...
	include_once('includes/classes/class_CompanyServices.php');
		...
		if (empty($customEvaluation)) {
			...
			$bidAcceptanceName = $bidAcceptanceArray['bidAcceptanceName'];
			$bidAcceptanceAmount = $bidAcceptanceArray['bidAcceptanceAmount'];
			$bidAcceptanceSplit = $bidAcceptanceArray['bidAcceptanceSplit'];
			$bidAcceptanceDue = $evaluationArray['bidAcceptanceDue'];

			$projectCompleteName = $projectCompleteArray['projectCompleteName'];

			$projectCompleteAmount = $projectCompleteArray['projectCompleteAmount'];
			$projectCompleteSplit = $projectCompleteArray['projectCompleteSplit'];
			...
			if ($bidAcceptanceArray['bidAcceptanceName'] == Null || $bidAcceptanceArray['bidAcceptanceName']==''){
			...
			if ($projectCompleteArray['projectCompleteName'] == Null || $projectCompleteArray['projectCompleteName']==''){
		...
		include_once('includes/classes/class_Mudjacking.php');
			...
			if (!empty($mudjackingArray)) {
				foreach($mudjackingArray as $row) {
					...
					$mudjackingDisplay .= '<strong>'.$mudjackingLocation.'</strong> - '.$mudjackingUpcharge.''.$mudjackingNotes.'<br/>';
				}
	...
	include_once('includes/classes/class_SumpPumps.php');
		...
		$dompdf->stream($firstName.'-'.$lastName.'-Bid-Preview',array('Attachment'=>0));//Display in Browser	
	} 
?>
