	...
	class Metrics {
		...
		public function getMetrics() {
			...
			if (!empty($this->companyID)) {
				...
				$st = $this->db->prepare("SELECT sales, calendarBgColor, SUM(days) as days, COUNT(bids) as bids FROM((
					SELECT p.projectSalesperson, CONCAT( u.userFirstName,' ', u.userLastName) AS sales, u.calendarBgColor, (DATEDIFF(b.bidAccepted, b.bidFirstSent)) as days, b.bidAccepted as bids
				...
				if ($st->rowCount()>=1) {
					...
					$table['cols'] = array(
					    ...
					    array('label' => 'Average Time From Bid To Sale', 'type' => 'number'),
					...
					$data['title'] = 'Average Time From Bid To Sale (Days)';
