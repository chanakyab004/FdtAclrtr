	...
	class BidNotSent {
		...
		public function getNotification() {
			...
			if (!empty($this->companyID)) {
				...
				$st = $this->db->prepare("SELECT 'BidNotSent' AS notificationType, c.firstName, c.lastName, e.evaluationFinalized AS time, e.evaluationID AS link FROM 
				...
				e.evaluationFinalized + INTERVAL 2 DAY < UTC_TIMESTAMP AND 
				...
				$st = $this->db->prepare("SELECT 'BidNotSent' AS notificationType, c.firstName, c.lastName, e.evaluationFinalized, e.evaluationID FROM 
