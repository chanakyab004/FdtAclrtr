<?php
	...
	class Schedule {
		...
		public function setSchedule($projectScheduleID, $projectID, $scheduledUserID, $scheduleType, $scheduledStart, $scheduledEnd, $scheduledByUserID) {
		...
		public function sendSchedule() {
			...
			if (!empty($this->projectID)) {
				...
				$st->bindParam(':scheduledByUserID', $this->scheduledByUserID);
				...
				if ($this->scheduleType == 'Evaluation') {
					...
					//Get Info for Email
					$secondST = $this->db->prepare("SELECT p.projectID, p.projectSalesperson, t.address, t.city, t.state, t.zip, m.firstName, m.lastName, m.email, m.customerID, m.unsubscribed, m.noEmailRequired,  c.companyID, c.companyName, c.companyAddress1, c.companyCity, c.companyState, c.scheduleEmailSendSales, c.companyZip, c.companyWebsite, c.companyLogo, c.companyColor, c.companyEmailReply, c.companyEmailFrom, c.companyEmailSchedule, u.userFirstName, u.userLastName, u.userEmail, u.userPhoto, u.userBio, u2.userFirstName AS salesFirstName,
