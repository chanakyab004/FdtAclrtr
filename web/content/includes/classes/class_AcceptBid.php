<?php
	...
	class Bid {
			...
			if (!empty($this->evaluationID)) {
				...
				if (empty($this->customEvaluation)) {
						...
						$stTwo = $this->db->prepare("
							SELECT `bidAccepted` FROM `evaluationBid` WHERE evaluationID = :evaluationID"
					...
					$st = $this->db->prepare("
						...
						$stTwo = $this->db->prepare("
							SELECT `bidAccepted` FROM `customBid` WHERE evaluationID = :evaluationID"
		...
		public function getUsersToUpdate(){
			if (!empty($this->evaluationID)) {
			$st = $this->db->prepare("SELECT * FROM((SELECT b.evaluationID, p.scheduledUserID as userID FROM `customBid` as b

				LEFT JOIN evaluation as e ON b.evaluationID = e.evaluationID
				LEFT JOIN projectSchedule as p ON e.projectID = p.projectID

				WHERE bidAccepted IS NOT NULL)

				UNION

				(SELECT b.evaluationID, p.scheduledUserID as userID FROM `evaluationBid` as b
				...
				(SELECT :evaluationID as evaluationID, userID FROM `user`

				WHERE `primary` = 1 OR `projectManagement` = 1 OR `bidVerification` = 1)

				) as t WHERE evaluationID = :evaluationID
    ...
    public function sendEmail($userEmail, $projectID, $customerFirstName, $customerLastName, $firstName, $lastName ){
                      ...
                      <table class=\"email-body_inner\" align=\"center\" width=\"570\" cellpadding=\"0\" cellspacing=\"0\">
                        ...
                        <tr>
                          <td class=\"content-cell\">
                            ...
                            <p>".$customerFirstName." ".$customerLastName. " has accepted your bid.</p>
                            <p>".$viewProjectLink."</p>
                          </td>
                        </tr>
                      </table>
      ...
      $altbody = "Hello ".$firstName.",
                  ".$customerFirstName." ".$customerLastName. " has accepted your bid.
                  ".$viewProjectLink."";
