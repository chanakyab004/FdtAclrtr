	...
	class InstallationComplete {
		...
		public function getStatus() {
			...
			if (!empty($this->companyID)) {
				...
				$sqlStatement = "(SELECT 'InstallationComplete' AS statusType, c.firstName, c.lastName, s.installationComplete AS time, j.projectID AS link, p.address, p.address2, p.city, p.state, p.zip, s.projectScheduleID, s.scheduleType, e.evaluationID FROM 
				...
				(SELECT 'InstallationComplete' AS statusType, c.firstName, c.lastName, s.installationComplete AS time, j.projectID AS link, p.address, p.address2, p.city, p.state, p.zip, s.projectScheduleID, s.scheduleType, e.evaluationID FROM 
