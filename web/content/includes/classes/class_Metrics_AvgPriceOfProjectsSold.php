	...
	class Metrics {
		...
		public function getMetrics() {
			...
			if (!empty($this->companyID)) {
				...
				$st = $this->db->prepare("SELECT sales, SUM(total) as total, COUNT(count) as count, calendarBgColor FROM ((
					SELECT p.projectSalesperson, CONCAT( u.userFirstName,  ' ', u.userLastName ) AS sales, bidTotal AS total, bidAccepted AS count, u.calendarBgColor
						...
						AND b.bidAccepted >=  :dateFrom
						AND b.bidAccepted <  :dateTo)
				...
				if ($st->rowCount()>=1) {
					...
					$table['cols'] = array(
					    ...
					    array('label' => 'Average Price of Projects Sold', 'type' => 'number'),
					...
					$data['title'] = 'Average Price of Projects Sold';
