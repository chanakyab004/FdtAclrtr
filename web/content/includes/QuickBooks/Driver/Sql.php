	...
	*/
	...
	protected function _oauthLoad($app_username, $app_tenant)
	{
		...
		if ($arr = $this->fetch($this->query("
			...
			FROM 
				" . $this->_mapTableName(QUICKBOOKS_DRIVER_SQL_OAUTHTABLE) . " 
			WHERE
				app_username = '%s' AND app_companyID = '%s' ", $errnum, $errmsg, null, null, array( $app_username, $app_tenant ))))
		{
			$this->query("
				UPDATE 
					" . $this->_mapTableName(QUICKBOOKS_DRIVER_SQL_OAUTHTABLE) . "
				SET
					touch_datetime = '%s' 
				WHERE
					app_username = '%s' AND app_companyID = '%s' ", $errnum, $errmsg, 0, 1, array( date('Y-m-d H:i:s'), $app_username, $app_tenant ));
		...
		if ($arr = $this->_oauthLoad($app_username, $app_tenant))
		{
			...
			return $this->query("
				...
				SET 
					...
					request_datetime = '%s'
				WHERE
					app_username = '%s' AND app_companyID = '%s' ", $errnum, $errmsg, null, null, array( $token, $token_secret, date('Y-m-d H:i:s'), $app_username, $app_tenant ));
		}
		else
		{
	...
	protected function _oauthAccessWrite($request_token, $token, $token_secret, $realm, $flavor)
	{
		...
		if ($arr = $this->_oauthRequestResolve($request_token))
		{
			...
			//Update Company Table to reflect quickbooks connection status
		    $this->query("
		    	UPDATE company SET 
				     quickbooksStatus = '1'
				WHERE companyID = (SELECT app_companyID
				FROM quickbooks_oauth
				WHERE oauth_request_token = '".$request_token."' ) ", $errnum, $errmsg, null, null, null);
	...
	protected function _oauthAccessDelete($app_username, $app_tenant)
	{
		...
		SET 
			quickbooksStatus = '2'
		WHERE
			companyID = '%s' ", $errnum, $errmsg, null, null, array($app_tenant));
		...
		$this->query("
			...
			WHERE
			app_username = '%s' AND 
			app_companyID = '%s' ", $errnum, $errmsg, null, null, array( $app_username, $app_tenant ));
	...
	protected function _initialize($init_options = array())
	{
		...
		*/
		...
		$def = array(
			...
			'app_companyID' => array( QUICKBOOKS_DRIVER_SQL_VARCHAR, 255 ), 
		...
		$keys = array(  );
		$uniques = array( array( 'app_username', 'app_companyID' ) );
