<?php
	...
	class GetProjectsWithEvaluations {
			...
			if (!empty($this->token)){
				...
				if ($st->rowCount()==1) {
					while ($row = $st->fetch((PDO::FETCH_ASSOC))) {
						...
						$this->getProjectsWithEvaluations($companyID);
					}
				}
				else{
		...
		public function getProjectsWithEvaluations($companyID) {
			...
			if (!empty($companyID)) {
				...
				WHERE m.companyID=:companyID AND p.projectCancelled IS NULL AND s.scheduleType = 'evaluation' AND s.scheduledStart < :viewEndDate AND s.scheduledEnd >= :viewStartDate");
				//write parameter query to avoid sql injections
