<?php
  ...
  class SaveProjectComplete {
        ...
        if ($st->rowCount()==1) {
          while ($row = $st->fetch((PDO::FETCH_ASSOC))) {
            ...
            if ($this->installationExists()){
              $this->saveProjectComplete($userID);
            }
            else{
    ...
    public function saveProjectComplete($userID) {
      
      if (!empty($userID) && !empty($this->completionDate) && !empty($this->projectID)) {

        $st = $this->db->prepare("UPDATE projectSchedule 
                                  SET installationComplete = :completionDate,
                                  installationCompleteRecordedDT = UTC_TIMESTAMP,
                                  installationCompleteRecordedByUserID = :userID
                                  WHERE projectID = :projectID AND scheduleType = 'installation'");
        //write parameter query to avoid sql injections
        $st->bindParam(":completionDate", $this->completionDate);
        $st->bindParam(":userID", $userID);
        $st->bindParam(":projectID", $this->projectID);
        if ($st->execute()) {
          $this->results = array('message' => 'success', 'projectID' => $this->projectID, 'installationComplete' => $this->completionDate, 'userID' => $userID); 
          }
        }
        else{
