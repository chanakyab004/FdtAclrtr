<?php
	...
	//$todaysDate = '2016-12-04';
	$emailBody = NULL;

	include_once($_SERVER['DOCUMENT_ROOT'] . '/includes/classes/class_AllCompanies.php');
		...
		if (!empty($companyArray)){
			foreach ( $companyArray as &$row) {
				...
				$isSubscriptionCancelled = $row['isSubscriptionCancelled'];
					...
					include_once($_SERVER['DOCUMENT_ROOT'] . '/includes/classes/class_SubscriptionPricing.php');
					...
					if (!empty($subscriptionNextBill)) {
						...
						if ($todaysDate >= $subscriptionNextBill && ($todaysDate < $subscriptionExpiration || $isSubscriptionCancelled != 1)) {
							...
							echo $companyName . ' Will Renew<br/>';
							...
							// include_once('includes/classes/class_UpdateSubscriptionDates.php');
					...
					include_once($_SERVER['DOCUMENT_ROOT'] . '/includes/classes/class_OpenTransactionAmount.php');
					...
					if (!empty($openTransactions)){
						...
						echo $companyName . ' Will Be Charged ' . $amountToCharge . '<br/>';
					}
					...
					// 	include_once('charge-customer-profile.php');
     ...
     //  						$authorizeTransactionID = $chargedCustomerProfile['authorizeTransactionID'];
					...
					// 		include_once('includes/classes/class_EditAllOpenTransactionsCharged.php');
		...
		// 		$Mail->addAddress('amanda@foundationaccelerator.com');
		// 	  	$Mail->isHTML( TRUE );
		// 	  	$Mail->Body = $emailBody;
